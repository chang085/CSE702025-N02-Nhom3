...
public class ViewCauThu {
    ...
    private static JButton addButton, changeButton, deleteButton, in4, sortMenuButton, addLatestMatchScoreButton,
            backButton, saveButton, filterMenuButton;
    private static JTextField nameField, nationalityField, matchField, goalField, salaryField, lastFiveMatchesField;
    private static JComboBox<String> genderField, dayComboBox, monthComboBox, yearComboBox, joinDayComboBox,
            joinMonthComboBox, joinYearComboBox, positionComboBox;

    public static void view(int maDoi) {
        data0 = null;
        validator = new DateValidator("dd/MM/uuuu");

        data = ControllerCauThu.loadData(maDoi);

        frame = new JFrame("Footballer");

        String[] col = { "Name", "Nationality", "Gender", "Date of Birth", "Date of Joining", "Position",
                "Matches", "Goals", "Agreed Salary", "Points from Last 5 Matches", "In the team" };
        ...
        addButton.addActionListener(e -> {
            ...
            ControllerCauThu.them(model, fields, nameField, nationalityField, positionComboBox, genderField,
                    joinDayComboBox, joinMonthComboBox, joinYearComboBox, matchField, goalField, salaryField,
                    dayComboBox, monthComboBox, yearComboBox, validator, lastFiveMatchesField, maDoi);
        });
        ...
        addLatestMatchScoreButton.addActionListener(e -> {
            ControllerCauThu.them5tran(table, model, maDoi);
        });

        in4.addActionListener(e -> {
            ControllerCauThu.in4(table, model, maDoi);
        });
        ...
        sortByGoals.addActionListener(e -> ControllerCauThu.sortTableByColumn(model, 7, false)); // Số bàn thắng ở cột
                                                                                                 // thứ 8 (index 7)
        sortBySalary.addActionListener(e -> ControllerCauThu.sortTableByColumn(model, 8, false)); // Lương ở cột thứ 9
                                                                                                  // (index 8)
        sortByMatches.addActionListener(e -> ControllerCauThu.sortTableByColumn(model, 6, false)); // Số trận ở cột thứ
                                                                                                   // 7 (index 6)
